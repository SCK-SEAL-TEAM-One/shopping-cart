...
func (cartService CartService) AddCart(submitedCart SubmitedCart) (string, error) {

	cart, err := cartService.CartRepository.GetCartByProductID(submitedCart.UserID, submitedCart.ProductID)

	act := "update"
	if err == sql.ErrNoRows {
		act = "add"
		cartService.CartRepository.CreateCart(submitedCart.UserID, submitedCart.ProductID, submitedCart.Quantity)
		return act, nil
	}
	err = cartService.CartRepository.UpdateCart(submitedCart.UserID, submitedCart.ProductID, submitedCart.Quantity+cart.Quantity)
	if err != nil {
